  ...
  const TypingIndicator = () => (
    ...
    >
      <div className="flex items-start gap-3">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#C4B5FD] to-[#A78BFA] flex items-center justify-center">
          <Bot className="w-4 h-4 text-[#491B6D]" />
        </div>
        <div className="bg-white/80 border border-white/60 rounded-2xl rounded-tl-md p-4">
          <div className="flex space-x-1">
            <motion.div
              ...
              className="w-2 h-2 bg-[#491B6D]/60 rounded-full"
            />
            <motion.div
              animate={{ scale: [1, 1.2, 1] }}
  ...
  return (
    <div className="h-full flex flex-col bg-transparent">
        ...
        {messages.length === 1 && !showWordInput && (
          ...
          >
            <div className="text-center space-y-2">
              <p className="text-sm" style={{ fontWeight: 600, color: '#491B6D' }}>
                어떤 도움이 필요하신가요?
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {tutorFeatures.map((feature, index) => (
                <motion.button
                  ...
                  className="p-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-[#A78BFA]/30 shadow-md active:shadow-lg transition-all duration-300 flex flex-col items-center justify-center text-center"
                >
                  <feature.icon className="w-6 h-6 mb-2" style={{ color: '#8B5CF6' }} />
                  <p className="text-xs mb-2" style={{ fontWeight: 600, color: '#491B6D' }}>
                    {feature.text}
                  </p>
                  <span 
        ...
        {showWordInput && selectedFeature && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 border border-[#A78BFA]/30 shadow-2xl w-full max-w-sm"
            >
              <div className="flex items-center gap-3 mb-4">
                <selectedFeature.icon className="w-6 h-6" style={{ color: '#8B5CF6' }} />
                <div>
                  <h3 className="text-sm" style={{ fontWeight: 700, color: '#491B6D' }}>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <motion.button
                  ...
                  style={{ 
                    backgroundColor: inputValue.trim() ? '#8B5CF6' : '#D1D5DB',
                    fontWeight: 600,
                    fontSize: '14px'
                  }}
                >
            ...
            <div className={`max-w-[85%] ${message.isUser ? 'order-1' : 'order-2'}`}>
              <div className={`flex items-start gap-3 ${message.isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`p-4 rounded-2xl shadow-sm ${
                  message.isUser
                    ? 'bg-[#491B6D] text-white rounded-tr-md'
                    : 'bg-white/90 rounded-tl-md border border-white/60'
                }`}
                  style={!message.isUser ? { color: '#491B6D' } : {}}
                >
                  <div className="space-y-2">
                    {message.isUser ? (
                      <div className="text-[12px] whitespace-pre-wrap leading-relaxed">{message.text}</div>
                    ) : (
      ...
      <div className="bg-white/90 backdrop-blur-sm border-t border-white/30 px-[20px] py-[16px]">
        ...
        <p className="text-xs mt-2 text-center" style={{ color: '#8B5CF6', opacity: 0.6 }}>
